...
iCNplot = function(iCNmat, ref, Gini, annotation = NULL, plot.dendrogram = TRUE, filename){
  ...
  png(paste0(filename, ".png"), width=2500, height=1600, pointsize=25)
  ...
  # 2) hclust
  if(plot.dendrogram){
    plot(rev(as.dendrogram(rclust)),leaflab="none",horiz=TRUE,axes=FALSE,yaxs="i")
  }

  # 3) Gini annotation
  anno.Gini = matrix(Gini[rclust$order], nrow = nrow(dat), ncol = 1)
  col.Gini = gplots::colorpanel(50, "#F7FBFF", "#084594")
  smart_image(anno.Gini, col=col.Gini, xaxs="i",yaxs="i",axes=FALSE)
  ...
  image(t(anno.chrom), col=c("gray", "black"), xaxs="i",yaxs="i",axes=FALSE)
  ...
  if(!is.null(annotation)){
    # 7) Customized annotation legend
    plot(0,0,type="n",axes=FALSE)
    # legend("center",legend=sort(unique(annotation)),
    #        col=col.anno, pch=15, bty="n", cex=1.5, title = paste(deparse(substitute(annotation))))
    legend("center",legend=sort(unique(annotation)),
