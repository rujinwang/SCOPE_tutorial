...
#' @import stats
#' @export
normalize_scope = function (Y_qc, gc_qc, K, norm_index, T,
  ...
  Ntotal <- apply(apply(Y.nonzero, 2, function(x){x/pseudo.sample}), 2, median)
  ...
  alpha0 = matrix(nrow=nrow(Y_qc), ncol=ncol(Y_qc)) # adjusted / absolute copy number
  ...
  fhat.temp=getfGC(gcfit.temp = gcfit.temp, gctemp = gc_qc,
    ...
    while (iter <= maxiter) {
      ...
      while (hiter <= maxhiter) {
        for (s in seq_len(nrow(Y_qc))) {
          temp=try(glm(formula = Y_qc[s,norm_index] ~ hhat[norm_index,] - 1,
                       offset = L[s,norm_index], family = poisson)$coefficients,
                   silent=TRUE)
          if(is.character(temp)){
            temp=lm(log(pmax(Y_qc[s,norm_index],1)) ~ hhat[norm_index,] - 1,
...
getfGCj = function(gcfit.tempj, gctemp, Yj, offsetj, T,
  ...
  if(draw.plot){
    ...
    smoothScatter(gctemp[-bin.filter], gcfit.tempj[-bin.filter],
                  main = 'Original', xlab = 'GC content', ylab = 'Y/beta/N/exp(gxh)')
  }
  for(Ti in T){
    if(Ti==1){
    ...
    if(Ti>=2){
      ...
      loe.fit.temp = loess((gcfit.tempj/(Z%*%as.matrix(seq_len(Ti)/2)))[-bin.filter]~gctemp[-bin.filter])
      fGCi = predict(loe.fit.temp, newdata = gctemp, se = TRUE)$fit
      temp = min(fGCi[!is.na(fGCi) & fGCi>0])
      fGCi[fGCi <= 0 | is.na(fGCi)] = temp
    ...
    if(Ti==1){
      ...
      loglik[which(T==Ti)] = sum(dpois(Yj[-bin.filter],
                                       lambda = (offsetj*fGCi*(Z%*%as.matrix(seq_len(Ti)/2)))[-bin.filter],
                                       log = TRUE))
    ...
    if(draw.plot){
      ...
      if(Ti == 1){
        ...
        for(k in seq_len(Ti)){
          points(gctemp[order(gctemp)], fGCi.plot[order(gctemp)]*k/2,
                 lty = 2, col = k, type = 'l', lwd = 2)
          points(gctemp[which((round(Z))[,k]==1)],
                 (gcfit.tempj)[which((round(Z))[,k]==1)],
...
getfGC = function(gcfit.temp, gctemp, Y, norm_index, offset, T, alpha, minCountQC){
  ...
  for(j in seq_len(ncol(Y))){
    ...
    if( j %in% norm_index){
      ...
      fGCj = getfGCj(gcfit.tempj = gcfit.temp[,j], gctemp = gctemp,
                     Yj = Y[,j], offsetj = offset[,j], T = T,
                     draw.plot = FALSE, alphaj = alpha[,j], minCountQC = minCountQC)
...
Estep = function(fGCi, vec_pi, Yj, offsetj){
  ...
  lambda =  matrix(nrow = nrow(P), ncol = ncol(P), data = offsetj*fGCi) *
    matrix(nrow = nrow(P),ncol = ncol(P), data = seq_len(length(vec_pi))/2, byrow = TRUE)
  P = dpois(matrix(nrow = nrow(P), ncol = ncol(P), data = Yj), lambda = lambda, log = TRUE) +
    matrix(nrow = nrow(P),ncol = ncol(P), data = log(vec_pi), byrow = TRUE)
  Z = matrix(nrow = length(fGCi), ncol = length(vec_pi))
  for(k in seq_len(length(vec_pi))){
    Z[,k] = 1/(1+apply(exp(apply(P, 2, function(x){x-P[,k]})[,-k, drop = FALSE]), 1, sum))
  }
  return(Z)
}
...
Mstep = function(Z, gcfit.tempj, gctemp){
  gcfit.temp = gcfit.tempj/(Z%*%as.matrix(seq_len(ncol(Z))/2))
