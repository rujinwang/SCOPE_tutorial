...
#'  copy number states.
#'
#' @param Y read depth matrix after quality control
...
#' @import stats
#' @export
normalize_scope = function (Y, gc_qc, K, norm_index, N, T, alpha0, beta0, minCountQC = 20) {
  ...
  gcfit.temp=Y/Nmat/beta0
  offset=Nmat*matrix(nrow = nrow(Y), ncol = ncol(Y), data = beta0, byrow = FALSE)
  fhat.temp=getfGC(gcfit.temp = gcfit.temp, gctemp = gc_qc, Y = Y, norm_index = norm_index, offset = offset, T = T, alpha = alpha0, minCountQC = minCountQC)
  ...
  for (ki in 1:length(K)) {
    ...
    HHTHRESH = 1e-05
    iter = 1
    fhat = matrix(nrow = nrow(Y), ncol = ncol(Y), data = 0)
    betahat = beta0
    betahatmat = matrix(nrow = nrow(Y), ncol = ncol(Y),
                        data = betahat, byrow = FALSE)
    ghat = matrix(0, nrow = nrow(Y), ncol = k)
    hhat = matrix(0, nrow = ncol(Y), ncol = k)
    ...
    while (iter <= maxiter) {
      ...
      if(iter>1){
        gcfit.temp=Y/Nmat/betahat/exp(ghat %*% t(hhat))
        offset=Nmat*matrix(nrow = nrow(Y), ncol = ncol(Y),
      ...
      betahatnew = apply((Y/(fhatnew * Nmat * exp(ghat %*% t(hhat))))[,norm_index], 1, median)
      ...
      logmat = log(pmax(Y, 1)) - L
      logmat = logmat - matrix(nrow = nrow(Y), ncol = ncol(Y), data = apply(logmat, 1, mean), byrow = FALSE)
      ...
      while (hiter <= maxhiter) {
        for (s in 1:nrow(Y)) {
          temp=try(glm(formula = Y[s,norm_index] ~ hhat[norm_index,] -
                         1, offset = L[s,norm_index], family = poisson)$coefficients,silent=TRUE)
          if(is.character(temp)){
            temp=lm(log(pmax(Y[s,norm_index],1)) ~ hhat[norm_index,] -
        ...
        for (t in 1:ncol(Y)) {
          hhatnew[t, ] = glm(formula = Y[, t] ~ ghat -
    ...
    AIC[ki] = 2 * sum(Y * log(pmax(Yhat.temp,1)) - Yhat.temp) -
      2 * (length(ghat) + length(hhat))
    BIC[ki] = 2 * sum(Y * log(pmax(Yhat.temp,1)) - Yhat.temp) -
      (length(ghat) + length(hhat)) * log(length(Y))
    RSS[ki] = sum((Y - Yhat.temp)^2/length(Y))
