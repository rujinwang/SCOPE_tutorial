...
#' @param T a scalar integer indicating the number of CNV group. If \code{T = 1}, assume all reads are from normal
...
#' @export
getfGCj_foreach = function(gcfit.tempj, gctemp, Yj, offsetj, T, draw.plot = NULL, alphaj, minCountQC){
  ...
  if(Ti==1){
    ...
    loglik = sum(dpois(Yj[-bin.filter], lambda = (offsetj*fGCi)[-bin.filter], log = T))
    ...
    loglik = sum(dpois(Yj[-bin.filter], lambda = (offsetj*fGCi*(Z%*%as.matrix((1:Ti)/2)))[-bin.filter], log = T))
