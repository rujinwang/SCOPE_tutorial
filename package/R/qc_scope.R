...
qc_scope = function (Y_raw, sampname_raw, ref_raw, QCmetric_raw,
  ...
  message("Removed ", sum(sampfilter1), " samples due to failed library preparation.")
  sampfilter2 = (QCmetric_raw[,'mapq20_prop'] < mapq20_thresh)
  message("Removed ", sum(sampfilter2), " samples due to low proportion of mapped reads.")
  ...
  message("Excluded ", sum(binfilter1), " bins due to extreme GC content.")
  binfilter2 = (mapp < mapp_thresh)
  message("Excluded ", sum(binfilter2), " bins due to low mappability.")
  ...
  pseudo.sample <- apply(Y.nonzero,1,function(x){exp(sum(log(x))/length(x))})
  ...
  binfilter3 = (bin.sum >= (median(bin.sum)-nMAD*mad(bin.sum))) & (bin.sum <= (median(bin.sum)+nMAD*mad(bin.sum)))
  Y = Y[binfilter3,]
  ref = ref[binfilter3]
  QCmetric = as.data.frame(QCmetric)
  message("There are ", ncol(Y), " samples and ", nrow(Y), " bins after QC step. ")
