...
segmentCBScs = function(Y, Yhat, sampname, ref, chr, mode = "integer", max.ns = 0) {
  ...
  if(!is.null(init.cs.finalmat) && nrow(init.cs.finalmat)>0){
    ...
    while (!all(keep_going==0) & num_ns <= max.ns) {
      ...
      keep_going = vapply(nested.output, function(z) z$is.nested, numeric(1))

      newchpts = NULL
      for (r in seq_len(length(keep_going))) {
        if(keep_going[r]){
          newchpts = rbind(newchpts, nested.output[[r]]$finalmat[,c(1,2)])
        } else{
    ...
    cs.finalmat = cs.finalmat[!is.na(cs.finalmat[,3]), , drop = FALSE]
    cs.finalmat = cs.finalmat[order(-cs.finalmat[,3]), , drop = FALSE]
  ...
  for (s in seq_len(nrow(cs.finalmat))){
    tau <- sort(unique(c(as.vector(cs.finalmat[seq_len(s), c(1,2)]), 1, nrow(Y))))
    m <- length(tau) - 2
    if(m > 0){
      ...
      muhat[1,] = round(apply(Y0[seq_len(tau[2]), ,drop = FALSE], 2, sum) / tau[2])
      rhat[1,] = round(2*(apply(Y0[seq_len(tau[2]), ,drop = FALSE], 2, sum)/apply(Yhat0[seq_len(tau[2]), ,drop = FALSE], 2, sum)))
      for(r in seq_len(m)){
        muhat[r+1,] = round(apply(Yhat0[(tau[r+1]+1):tau[r+2], ,drop = FALSE], 2, sum) / (tau[r+2] - tau[r+1] + 1))
        rhat[r+1,] = round((apply(Y0[(tau[r+1]+1):tau[r+2], ,drop = FALSE], 2, sum)/apply(Yhat0[(tau[r+1]+1):tau[r+2], ,drop = FALSE], 2, sum)))
      }
      ...
      cs.temp = create_chptsmat(cs.finalmat[seq_len(s),c(1,2),drop = FALSE], chpts0)
      L = matrix(data = NA, ncol = N, nrow = nrow(cs.temp))
      for (r in seq_len(nrow(L))) {
        yact.temp = apply(Y[cs.temp[r,1]:cs.temp[r,2], ,drop = FALSE], 2, sum)
        lambda.temp = apply(Yhat[cs.temp[r,1]:cs.temp[r,2], ,drop = FALSE], 2, sum)
        yact.temp[lambda.temp < 20] <- 20
        lambda.temp[lambda.temp < 20] <- 20
        L[r,] = (1-round(2*yact.temp/lambda.temp)/2)*lambda.temp +
          log((round(2 * (yact.temp/lambda.temp)) + 1e-04)/2.0001) * yact.temp
      }
  ...
  cs.finalmat <- (cbind(cs.finalmat, cs.mBIC)[seq_len(which.max(cs.mBIC)), ,drop = FALSE])
  ...
  return(list(poolcall = poolcall, finalcall = finalcall,
              image.orig = image.orig, image.seg = image.seg,
              iCN = iCN))
}
...
create_chptsmat = function(mat1, st_end){
    ...
    for (r in 2:nrow(mat1)) {
      if(mat1[r,1] != mat1[r-1,2] + 1){
        newchptsmat = rbind(newchptsmat,
                            matrix(data = c(mat1[r-1,2] + 1, mat1[r,1] - 1, mat1[r,]),
                                   ncol = 2, byrow = TRUE))
      }else{
  ...
  if(mat1[nrow(mat1),2] != end){
    newchptsmat = rbind(newchptsmat, matrix(data = c(mat1[nrow(mat1),2]+1, end),
                                            nrow = 1, ncol = 2))
  }
  return(newchptsmat)
}
...
compute_cs_lratio = function(Y, Yhat, sampname, this.chpts, msgprint = TRUE){
  ...
  if(this.chpts[2] - this.chpts[1] < 2){
    ...
    if (sum(Z > 0) > 0) {
      ...
      finalmat <- finalmat[finalmat[,2] - finalmat[,1] > 1, ,drop = FALSE]
    ...
    if(!is.null(finalmat) && nrow(finalmat)>1){
      finalmat = create_chptsmat(finalmat[,c(1,2),drop = FALSE], this.chpts)
    }
...
search_cs_nested = function(Y, Yhat, sampname, chptsmat){
  ...
  for (r in seq_len(nrow(chptsmat))) {
    nest.list[[r]] = compute_cs_lratio(Y[chptsmat[r,1]:chptsmat[r,2], , drop = FALSE],
                                       Yhat[chptsmat[r,1]:chptsmat[r,2], , drop = FALSE],
                                       sampname, chptsmat[r,], msgprint = FALSE)
  }
  return(nest.list)
}
