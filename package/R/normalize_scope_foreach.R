...
#' @export
normalize_scope_foreach = function (Y_qc, gc_qc, K, norm_index, T, ploidyInt,
                                    beta0, minCountQC = 20, nCores = NULL) {
...
getfGCj_foreach = function(gcfit.tempj, gctemp, Yj, offsetj, T,
  ...
  if(Ti==1){
    ...
    loglik = sum(dpois(Yj[-bin.filter],
                       lambda = (offsetj*fGCi)[-bin.filter],
                       log = TRUE))
    BIC = 2*loglik-(length(gcfit.tempj)-df)*log(length(gcfit.tempj))
  } else{
    ...
    loglik = sum(dpois(Yj[-bin.filter],
                       ...
                       log = TRUE))
    BIC = 2*loglik-(length(gcfit.tempj)-df+Ti-1)*log(length(gcfit.tempj))
  }

  if(draw.plot){
    smoothScatter(gctemp[-bin.filter],gcfit.tempj[-bin.filter],
                  xlab = 'GC content', ylab = 'Y/beta/N/exp(gxh)',nrpoints  =  0, main = paste('T =',Ti))
    if(Ti == 1){
      ...
      for(k in seq_len(Ti)){
        ...
        points(gctemp[which((round(Z))[,k]==1)], (gcfit.tempj)[which((round(Z))[,k]==1)],
...
getfGC_foreach = function(gcfit.temp, gctemp, Y, norm_index,
                          offset, T, alpha, minCountQC, nCores){
  ...
  for(j in seq_len(ncol(Y))){
    ...
    if( j %in% norm_index){
      ...
      TList <- foreach(Ti = T,
                       .export = c("fitGC", "Estep", "Mstep", "getfGCj_foreach")) %dopar% {
