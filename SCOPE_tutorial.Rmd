...
knitr::include_graphics("/Users/rujin/Dropbox/writing/SCOPE_Git//Figures/Figure1_SCOPE_outline.jpg")
...
## 2.1 Pre-preparation
SCOPE provides a pre-stored whole-genome sequencing `.bed` file, with a fixed 500kb genomic interval length. SCOPE enables reconstruction of user-defined genome-wide consecutive bins prior to downstream analysis. Make sure that all chromosomes are named consistently and be concordant with `.bam` files. SCOPE processes the entire genome altogether. Use function `getbambed_scope()` to finish the pre-preparation step. 
```{r, eval=FALSE}
library(SCOPE)
bedFile <- system.file("extdata", "scWGA500kbsort.bed", package = "SCOPE")
bamfolder <- "/pine/scr/r/u/rujin/Navin_et_al_Nature_2011/align/"
...
Note that SCOPE can also be adapted to the mouse genome (mm10) in a similar way (see [CODEX2 for mouse genome](https://github.com/yuchaojiang/CODEX2/blob/master/README.md)). For unknown reference assembly without pre-calculated mappability track, refer to [CODEX2: mappability pre-calculation](https://github.com/yuchaojiang/CODEX2/blob/master/mouse/mapp.R).

## 2.3 Getting coverage
Obtain either single-end or paired-end sequencing read depth matrix. SCOPE, by default, adopts a fixed binning method to compute the depth of coverage while removing reads that are mapped to multiple genomic locations and to "blacklist" regions. This is followed by an additional step of quality control to remove bins with extreme mappability to avoid erroneous detections. Specifically, "blacklist" bins, including [segmental duplication regions](http://humanparalogy.gs.washington.edu/build37/data/GRCh37GenomicSuperDup.tab) and [gaps in reference assembly](https://gist.github.com/leipzig/6123703) from telomere, centromere, and/or heterochromatin regions. 
```{r, eval=FALSE}
# Get segmental duplication regions
seg.dup = read.table(system.file("extdata", "GRCh37GenomicSuperDup.tab", package = "SCOPE"), head = T)
seg.dup = seg.dup[!is.na(match(seg.dup[,1], paste('chr', c(1:22, 'X', 'Y'), sep = ''))),]
seg.dup = GRanges(seqnames = seg.dup[,1], ranges = IRanges(start=seg.dup[,2], end = seg.dup[,3]))
# Get hg19 gaps
gaps = read.table(system.file("extdata", "hg19gaps.txt", package = "SCOPE"), head = T)
...
Y.nonzero.sim <- Y_sim[apply(Y_sim, 1, function(x){!any(x==0)}),]
pseudo.sample.sim <- apply(Y.nonzero.sim,1,function(x){exp(1/length(x)*sum(log(x)))})
Ntotal.sim <- apply(apply(Y.nonzero.sim, 2, function(x){x/pseudo.sample.sim}), 2, median)
N.sim <- round(Ntotal.sim/median(Ntotal.sim)*median(colSums(Y_sim)))
Nmat.sim <- matrix(nrow = nrow(Y_sim), ncol = ncol(Y_sim), data = N.sim, byrow = TRUE)
...
normObj.sim <- normalize_codex2_ns_noK(Y_qc =Y_sim,
                                       ...
                                       N = N.sim)
Yhat.noK.sim=normObj.sim$Yhat[[1]]
beta.hat.noK.sim=normObj.sim$beta.hat[[1]]
fGC.hat.noK.sim=normObj.sim$fGC.hat[[1]]

# Ploidy initialization
ploidy.sim =  PreEst_ploidy(Y = Y_sim, Yhat = Yhat.noK.sim, ref = ref_sim)

# Get initialization
gcfit.temp.sim = Y_sim/Nmat.sim/beta.hat.noK.sim
alpha.sim0 = matrix(nrow=nrow(Y_sim), ncol=ncol(Y_sim)) # adjusted / absolute copy number
for(j in 1:ncol(alpha.sim0)){
  loe.fit=loess(gcfit.temp.sim[,j]~ref_sim$gc)
  gcfit.null.sim=loe.fit$fitted/(ploidy.sim[j]/2)
  alpha.sim0[,j] = gcfit.temp.sim[,j]/gcfit.null.sim*2
}

offset.sim=Nmat.sim*matrix(nrow = nrow(Y_sim), ncol = ncol(Y_sim), data = beta.hat.noK.sim, byrow = FALSE)

# If using high performance clusters, parallel computing is easy and improves computational efficiency. Simply use normalize_scope_foreach() instead of normalize_scope(). All parameters are identical. 
normObj.scope.sim = normalize_scope_foreach(Y_qc = Y_sim, gc_qc = ref_sim$gc, K = 1:3,
                                            norm_index = which(Gini<=0.12), N = N.sim, T = 1:7,
                                            alpha0 = alpha.sim0, beta0 = beta.hat.noK.sim)
# normObj.scope.sim = normalize_scope(Y_qc = Y_sim, gc_qc = ref_sim$gc, K = 1:3,
#                                     norm_index = which(Gini<=0.12), N = N.sim, T = 1:7,
#                                     alpha0 = alpha.sim0, beta0 = beta.hat.noK.sim)
...
```{r, eval=FALSE}
choiceofT(Y = Y_sim, gcfit.temp = gcfit.temp.sim, gctemp = ref_sim$gc,
          norm_index = which(Gini<=0.12), offset = offset.sim, T = 1:7, alpha = alpha.sim0,
          filename = "choiceofTdemo.pdf")
```

```{r, out.width = "500px", include=TRUE, fig.align="center", echo=FALSE}
knitr::include_graphics("/Users/rujin/Dropbox/writing/SCOPE_Git/Figures/demo_EM_fitting.png")
# knitr::include_graphics("C:/Users/rujin/Dropbox/writing/SCOPE_Git/Figures/demo_EM_fitting.png")
```
...
```{r, eval=FALSE}
iCNplot(iCNmat = iCN, ref = ref_sim, Gini = Gini, filename = "iCNplot")
```

We demonstrate SCOPE on scDNA-seq data of two breast cancer patients (T10 and T16) from [Navin et al. 2011 Nature](https://www.nature.com/articles/nature09807). Figures below give heatmaps of genome-wide estimated integer copy numbers across all cells from T10 and T16. 

```{r, out.width = "800px", include=TRUE, fig.align="center", echo=FALSE}
knitr::include_graphics("/Users/rujin/Dropbox/writing/SCOPE_Git/Figures/Navin_et_al_heatmap_T10.png")
# knitr::include_graphics("C:/Users/rujin/Dropbox/writing/SCOPE_Git/Figures/Navin_et_al_heatmap_T10.png")
```

```{r, out.width = "800px", include=TRUE, fig.align="center", echo=FALSE}
knitr::include_graphics("/Users/rujin/Dropbox/writing/SCOPE_Git/Figures/Navin_et_al_heatmap_T16.png")
# knitr::include_graphics("C:/Users/rujin/Dropbox/writing/SCOPE_Git/Figures/Navin_et_al_heatmap_T16.png")
```
