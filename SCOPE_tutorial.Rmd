...
```{r, eval=FALSE}
segment_cs = vector('list',6)
names(segment_cs) = c("poolcall", "finalcall", "finalcall_CODEX2", "image.orig", "image.seg", "iCN")
for (chri in paste0("chr", 1:22)) {
  message('\n', chri, '\n')
  temp_segment_cs = segmentCBScs(Y = Y[which(seqnames(ref)==chri),], 
                                 Yhat = Yhat[which(seqnames(ref)==chri),], 
                                 sampname = colnames(Y), ref = ref[which(seqnames(ref)==chri)], lmax = 100)
