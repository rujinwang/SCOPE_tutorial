...
The whole-genome mappability track on human hg38 assembly is also stored in SCOPE package. For more details on mappability calculation, please refer to [CODEX2 for hg38](https://github.com/yuchaojiang/CODEX2/blob/master/README.md). 
...
# Post-coverage cell QC
# remove samples whose library prep failed 
lib.nofail = which(apply(Y,2,sum)>0)
QCmetric=QCmetric[lib.nofail,]
sampname=sampname[lib.nofail]
Y=Y[,lib.nofail]
# remove samples with low proportion of mapped reads
filter = which(QCmetric[,'mapq20_prop']>=0.6)
QCmetric = QCmetric[filter,]
sampname = sampname[filter]
Y=Y[,filter]

# We recommend removal of bins whose sum of read counts adjusted by library size are outside 3 MAD from the median. 
# get library size factor N
...
# Sum of read counts adjusted by N for each bin
bin.sum=apply(Y/Nmat,1,sum)
hist(bin.sum, 100, main = "Sum of read counts adjusted by N for each bin")
# bin QC: 3 MAD from median
abline(v=(median(bin.sum)-3*mad(bin.sum)), lty=2, col=2)
abline(v=(median(bin.sum)+3*mad(bin.sum)), lty=2, col=2)
Y=Y[(bin.sum>=(median(bin.sum)-3*mad(bin.sum))) & (bin.sum <=(median(bin.sum)+3*mad(bin.sum))),]
ref=ref[(bin.sum>=(median(bin.sum)-3*mad(bin.sum))) & (bin.sum <=(median(bin.sum)+3*mad(bin.sum)))]
gc = gc[(bin.sum>=(median(bin.sum)-3*mad(bin.sum))) & (bin.sum <=(median(bin.sum)+3*mad(bin.sum)))]
Nmat = Nmat[(bin.sum>=(median(bin.sum)-3*mad(bin.sum))) & (bin.sum <=(median(bin.sum)+3*mad(bin.sum))),]
...
```{r, eval=FALSE}
# Re-calculate library size after cell/bin QC
Y.nonzero <- Y[apply(Y, 1, function(x){!any(x==0)}),]
...
ploidy =  PreEst_ploidy(Y = Y, Yhat = Yhat.noK, ref = ref)
ploidy
...
offset=Nmat*matrix(nrow = nrow(Y), ncol = ncol(Y), data = betahat, byrow = FALSE)
...
```{r, eval=FALSE}
choiceofT(Y = Y, gcfit.temp = gcfit.temp, gctemp = ref$gc, 
          norm_index = which(Gini<=0.12), offset = offset, T = 1:7, alpha = alpha0, 
          filename = paste0("choiceofTdemo.pdf"))
```
...
SCNP = matrix(nrow = length(RCNP), ncol = length(cploidy), data = RCNP) * matrix(nrow = length(RCN
P), ncol = length(cploidy), data = cploidy, byrow = T)
...
```{r, eval=FALSE}
segment_cs = vector('list',7)
names(segment_cs) = c("poolcall", "finalcall", "finalcall.idv", "image.orig", "image.seg", "iCN", "iCN.idv")
for (chri in paste0("chr", 1:22)) {
  message('\n', chri, '\n')
  temp_segment_cs = segmentCBScs(Y = Y[which(seqnames(ref)==chri),], 
                                 Yhat = Yhat[which(seqnames(ref)==chri),], 
                                 sampname = colnames(Y), ref = ref[which(seqnames(ref)==chri)], 
                                 mode = "integer", segment.idv = FALSE, max.ns = 1)
  temp_segment_cs$poolcall[,'st'] = temp_segment_cs$poolcall[,'st'] + min(which(seqnames(ref)==chri)) - 1
  temp_segment_cs$poolcall[,'end'] = temp_segment_cs$poolcall[,'end'] + min(which(seqnames(ref)==chri)) - 1
  temp_segment_cs$finalcall[,'st_bin'] = temp_segment_cs$finalcall[,'st_bin'] + min(which(seqnames(ref)==chri)) - 1
  temp_segment_cs$finalcall[,'ed_bin'] = temp_segment_cs$finalcall[,'ed_bin'] + min(which(seqnames(ref)==chri)) - 1
  
  segment_cs = mapply(rbind, segment_cs, temp_segment_cs, SIMPLIFY=FALSE)
}
...
```{r, eval=FALSE}
iCNplot(iCNmat = iCN, ref = ref, Gini = Gini, filename = "iCNplot")
```
