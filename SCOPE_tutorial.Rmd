...
coverageObj <- getcoverage.scDNA(bambedObj, mapqthres = 40, mask.ref, seq='paired-end')
Y_raw <- coverageObj$Y
```
...
Yhat.noK=normObj$Yhat
beta.hat.noK=normObj$beta.hat
fGC.hat.noK=normObj$fGC.hat
N = normObj$N
...
normObj.scope = normalize_scope_foreach(Y_qc = Y, gc_qc = ref$gc,
                                            ...
                                            beta0 = beta.hat.noK, nCores = 2)
# normObj.scope = normalize_scope(Y_qc = Y, gc_qc = ref$gc,
#                                     K = 1, ploidyInt = ploidy,
#                                     norm_index = which(Gini<=0.12), T = 1:7,
#                                     beta0 = beta.hat.noK)
...
```{r, eval=FALSE}
choiceofT(Y_qc = Y, gc_qc = ref$gc, norm_index = which(Gini<=0.12), T = 1:7,
          ploidyInt = ploidy, beta0 = beta.hat.noK,
...
for (chri in chrs) {
  message('\n', chri, '\n')
  segment_cs[[chri]] = segmentCBScs(Y = Y,
                                    Yhat = Yhat,
                                    sampname = colnames(Y),
                                    ref = ref,
                                    chr = chri,
...
```{r, eval=FALSE}
iCNplot(iCNmat = iCN, ref = ref, Gini = Gini, filename = "iCNplotdemo")
```
